...
//  Mash-iOS
//
//  Created by Samy Hajal on 4/8/15.
//  Copyright (c) 2015 Mash. All rights reserved.
//

#import <Foundation/Foundation.h>
#import "PhaseTimeShifting.h"
...
@implementation PhaseTimeShifting {
    
}

+ (void)processing:(NSString*)path :(int)bpm : (int) otherBpm{
    float bpmRatio = bpm / otherBpm;
    // Open the input file.
    SuperpoweredDecoder *decoder = new SuperpoweredDecoder(false);
    const char *openError = decoder->open([[[NSBundle mainBundle] pathForResource:path ofType:@"mp3"] fileSystemRepresentation], false, 0, 0);
    if (openError) {
        NSLog(@"%s", openError);
        delete decoder;
        return;
    ...
    timeStretch->setRateAndPitchShift(bpmRatio, 0); // Audio will be 10% faster.
